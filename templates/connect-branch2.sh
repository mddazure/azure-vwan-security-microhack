...
az network vpn-site create --name onprem2 -g vwan-security-microhack-hub-rg --ip-address $vnetgwtunnelip1 --virtual-wan microhack-vwan --location northeurope --device-model VNETGW --device-vendor Azure --asn $vnetgwasn --bgp-peering-address $vnetgwbgpip1 --link-speed 100 --with-link true
...
hubgwtunneladdress=$(az network vpn-gateway show --name microhack-we-hub-vng  -g vwan-security-microhack-hub-rg --query "bgpSettings.bgpPeeringAddresses[?ipconfigurationId == 'Instance0'].tunnelIpAddresses[0]" --output tsv)
echo "Hub GW Tunnel address:" $hubgwtunneladdress
hubgwbgpaddress=$(az network vpn-gateway show --name microhack-we-hub-vng  -g vwan-security-microhack-hub-rg --query "bgpSettings.bgpPeeringAddresses[?ipconfigurationId == 'Instance0'].defaultBgpIpAddresses" --output tsv)
...
az network local-gateway create -g vwan-security-microhack-spoke-rg -n lng2 --gateway-ip-address $hubgwtunneladdress --location swedencentral --asn $hubgwasn --bgp-peering-address $hubgwbgpaddress

echo "# VNET GW: connect from vnet gw to local network gateway"
az network vpn-connection create -n onprem2-to-we-hub --vnet-gateway1 vnet-gw-onprem2 -g vwan-security-microhack-spoke-rg --local-gateway2 lng2 -l swedencentral --shared-key $sharedkey --enable-bgp
